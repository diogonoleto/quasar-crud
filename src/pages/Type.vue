<template>
  <div>
    <q-table
      ...
      hide-pagination
      flat
      v-show="Object.keys(types).length !== 0 && Object.keys(type).length == 0"
    >
      <template v-slot:body="props">
        <q-tr :props="props" @click="type = props.row">
    ...
    <q-card class="no-border-radius" v-if="Object.keys(type).length !== 0">
      <q-toolbar class="text-uppercase">
        <q-toolbar-title>{{ type.name }}</q-toolbar-title>
        <q-space />
        <q-btn
          round
          dense
        ...
        <q-btn round dense flat color="grey-8" icon="close" @click="type = {}">
...
export default {
  ...
  created() {
    ...
    this.$q.loading.show({
      message: online
        ? 'Carregando informações das tipos...'
    ...
    this.$store.dispatch('type/typeList');
  },
  computed: {
  ...
  methods: {
    ...mapActions('type', ['typeCreateOrUpdate', 'typeDelete']),
    sItem(prop) {
      this.type = this.types.find(e => e.id == prop);
    },
    async save(form) {
      this.$q.loading.show({
      ...
      try {
        await this.typeCreateOrUpdate(form);
      } catch (err) {
        this.$q.notify({
    ...
    async sDelete() {
      ...
      try {
        await this.typeDelete({ id: this.type.id });
      } catch (err) {
        this.$q.notify({
  ...
  watch: {
    ...
    dialog() {
      if (this.dialog) {
        this.header.title = 'NOVO TIPO';
        if (this.type && this.type.id) {
          this.header.title = 'EDITAR TIPO';
          Object.keys(this.items).forEach(d => {
            this.items[d].value = this.type[d];
          });
        } else {
          Object.keys(this.items).forEach(d => {
            this.items[d].value = '';
    ...
    dialogDelete() {
      this.header.title = 'O tipo será excluido, você tem certeza?';
      Object.keys(this.items).forEach(d => {
        this.items[d].value = this.type[d];
      });
    }
